...
public class OffreService implements OffreServiceRemote{
	...
	public void autoriser(int id){
		int query=entityManager.createQuery("UPDATE Offre SET auto=1 where ID="+id).executeUpdate();
	}
	
	public ArrayList<Offre> afficherOffreNon() {
		TypedQuery<Offre> query=entityManager.createQuery("Select p from Offre p Where auto=0",Offre.class);
		return (ArrayList<Offre>) query.getResultList();
	}
}
